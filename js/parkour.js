...
// 跑酷起点固定在高空，远离体素地形，避免与山丘/树木碰撞
const PARKOUR_START_Y = 55;          // 跑酷路径 Y（方块顶面 = START_Y，玩家脚 = START_Y+1）
const PARKOUR_FALL_OFFSET = 15;      // 掉落超过起点下方 15 格判定失败
// 自动跳跃：检测前方障碍/间隙自动起跳（可关闭，让玩家手动操作）
const PARKOUR_AUTO_JUMP = true;
...
export class ParkourManager {
  ...
  /**
   * 自动跳跃检测：判断前方是否需要起跳
   * - 前方 1-2 格有高于脚下的方块（台阶/墙）→ 跳
   * - 前方 2-3 格脚下无方块（间隙/断层）→ 跳
   * 返回 true 表示应起跳
   */
  _shouldAutoJump(player) {
    const px = Math.floor(player.position.x);
    const py = Math.floor(player.position.y);       // 脚下方块 Y
    ...
    for (let dz = 1; dz <= 3; dz++) {
      const fz = pz - dz;
      // 前方脚下方块
      const footBlock = world.getBlock(px, py, fz);
      // 前方头部高度方块（y+1, y+2）
      const headBlock = world.getBlock(px, py + 1, fz);
      const highBlock = world.getBlock(px, py + 2, fz);

      // 1. 前方有高墙（头部或更高位置有实体方块）→ 起跳
      if (isSolid(headBlock) || isSolid(highBlock)) {
        return true;
      }
      // 2. 前方脚下是空气（间隙/断层）→ 起跳（但只检测 2 格内，太远的不跳）
      if (dz <= 2 && !isSolid(footBlock)) {
        return true;
      }
    }
    return false;
  ...
  update(dt, player) {
    ...
    // === 跳跃（空格手动 / 自动检测障碍） ===
    const wantManualJump = player.keys['Space'] || player.keys['KeyK'];
    const wantAutoJump = PARKOUR_AUTO_JUMP && player.onGround && this._shouldAutoJump(player);
    if ((wantManualJump || wantAutoJump) && player.onGround) {
