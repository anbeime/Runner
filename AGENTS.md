...
### voxel.js
- **方块类型**: AIR, GRASS, DIRT, STONE, SAND, WOOD, LEAVES, WATER, BILIBILI_PINK, SNOW_GRASS, COBBLESTONE, MOSSY_COBBLESTONE, SANDSTONE, DIAMOND, GLASS, POPPY, DANDELION, ALLIUM, OXEYE_DAISY, WHITE_TULIP (共21种)
- **纹理图集**: Canvas 绘制 16x16 像素纹理，共多种纹理，合并为图集贴图
- **Chunk 类**: 16x48x16 方块数据存储 + BufferGeometry 网格生成（仅渲染暴露面），含空区块快速跳过
- **World 类**: 区块管理、地形生成（噪声高度图 + 树木生成 + 花朵生成）、动态渲染距离区块加载/卸载、**BILIBILI 文字立墙生成**
- **生物群系系统**: PLAINS（普通平原）、SNOW（雪地）、DESERT（沙地沙漠）三种生物群系，影响地表方块类型和树木种类
...
- **云彩系统**: 多层云彩粒子渲染，自动移动动画
- **天空系统**: 渐变天空背景，随时间变化
- **物品栏**: 14种可放置方块类型，数字键1-9和滚轮切换
- **TouchController**: 移动端虚拟摇杆、触屏视角、操作按钮
